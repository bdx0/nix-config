...
let
  inherit (lib)
    any attrValues concatMapStrings concatStringsSep const elem escapeShellArgs
    filterAttrs getName isString literalExpression mapAttrs mapAttrsToList
    mkAfter mkBefore mkDefault mkEnableOption mkIf mkMerge mkOption
...
in {
  ...
  options.bdx0.services.postgresql = {
        ...
        submodule {
          ...
          options = {
            shared_preload_libraries = mkOption {
              type =
                nullOr (coercedTo (listOf str) (concatStringsSep ", ") str);
              default = null;
  ...
  # meta.maintainers = with lib.maintainers; [ bdx0 ];
}
